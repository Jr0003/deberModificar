...
public class Main {
	...
	static Curso geteliminarCruso(int id) {
		Session session = sessionFactory.openSession();
		Query query = session.createQuery("from Curso where id=:id");
		query.setParameter("id",id);
		List<Curso> curso =(List<Curso>)query.getResultList();
		if(curso.size() !=0){
			return curso.get(0);
		}
		return null;
	...
	{
		Curso curso = geteliminarCruso(id);
		if(curso!=null) {
			Session session = sessionFactory.openSession();
			session.beginTransaction();
    ...
    static Estudiante geteliminarEstudiante(int id) {
		Session session = sessionFactory.openSession();
	...
	{
		Estudiante estudiantes = geteliminarEstudiante(id);
		if(estudiantes!=null) {
			Session session = sessionFactory.openSession();
			session.beginTransaction();
